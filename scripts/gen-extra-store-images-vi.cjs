...
const out = path.resolve('store-assets/android/vi-VN/images/extras');
fs.mkdirSync(out, { recursive: true });

const items = [
  {
    name: 'feature-1024x500-vi-a.png', w: 1024, h: 500, bg: '#0B5A9F',
    title: 'UVI LiveScore', sub: 'Ti so truc tiep, lich dau va chi tiet tran'
  },
  {
    name: 'feature-1024x500-vi-b.png', w: 1024, h: 500, bg: '#1D4ED8',
    title: 'UVI LiveScore', sub: 'Cap nhat lien tuc cac giai dau hang dau'
  },
  {
    name: 'icon-512x512-vi-a.png', w: 512, h: 512, bg: '#0F172A',
    title: 'TRUC TIEP', sub: 'UVI'
  },
  {
    name: 'icon-512x512-vi-b.png', w: 512, h: 512, bg: '#1E3A8A',
    title: 'TI SO', sub: 'UVI'
  },
];
...
<svg width="${it.w}" height="${it.h}" xmlns="http://www.w3.org/2000/svg">
  ...
  <text x="116" y="410" font-size="40" font-family="Arial" fill="#1D4ED8" font-weight="800">${it.title}</text>
...
(async () => {
  ...
  console.log(`Generated ${items.length} Vietnamese images in ${out}`);
})();
